...
def parse_count(text):
    ...
    For example, if text is "A, B, C" returns 3.
    """
    t = str(text).strip()
    if t == "":
        return 0
...
def count_direct_items(text):
    """
    Count only the comma‑separated items that contain "(RelationType = Direct)".
    """
    t = str(text).strip()
    if t == "":
        return 0
    return len([x for x in t.split(",") if x.strip() != "" and "(RelationType = Direct)" in x])

def build_summary(location, online_outage, ci_val, bc_val, nonbc_val):
    """
    Build the summary string for Column 1:
      - Location is trimmed. If blank, it becomes "" so that the joined result starts with a comma.
      - OnLine/Outage is trimmed.
      - CI count is taken as the total count of items in the CI field.
      - BC and NON BC counts are taken only from items containing "(RelationType = Direct)".
    Then join all parts with ", ".
    """
    parts = []
    # For location: trim. If blank, use "".
    loc = location.strip() if isinstance(location, str) else ""
    onl = online_outage.strip() if isinstance(online_outage, str) else ""
    parts.append(loc)  # May be empty if location was blank.
    parts.append(onl)
    ...
    # Join with comma and space. This may yield a leading comma if loc is empty.
    return ", ".join(parts)

def generate_formatted_excel(df):
    output = BytesIO()
    
    # Replace blank (NaN) cells with a space.
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for idx, row in df.iterrows():
            ...
            # If both dates are the same, show one; otherwise, show the range.
            ...
            # Split bc_content by newline if available; otherwise by comma.
            if "\n" in bc_content:
            ...
            for item in items:
                item = item.strip()
                if item.startswith("("):  # ignore headers like "(12 BC)"
                    continue
